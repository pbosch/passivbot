...
def calc_min_entry_qty(price, inverse, qty_step, min_qty, min_cost, contract_multiplier) -> float:
    return max(min_qty, round_up(min_cost * (price / contract_multiplier if inverse else 1 / price), qty_step))


@njit
def calc_qty_from_margin(margin, price, inverse, qty_step, contract_multiplier, leverage):
    if inverse:
...
@njit
def calc_initial_entry_qty(balance, price, available_margin, volatility, inverse, qty_step, min_qty, min_cost,
                           contract_multiplier, leverage, qty_pct, volatility_qty_coeff) -> float:
    min_entry_qty = calc_min_entry_qty(price, inverse, qty_step, min_qty, min_cost, contract_multiplier)
    if inverse:
        qty = round_dn(min(available_margin * leverage * price / contract_multiplier, max(min_entry_qty, (
                (balance / contract_multiplier) * price * leverage * qty_pct * (
                1 + volatility * volatility_qty_coeff)))), qty_step)
    else:
        qty = round_dn(min(available_margin * leverage / price, max(min_entry_qty, (
                (balance / price) * leverage * qty_pct * (1 + volatility * volatility_qty_coeff)))), qty_step)
    return qty if qty >= min_entry_qty else 0.0


@njit
def calc_reentry_qty(psize, price, available_margin, inverse, qty_step, min_qty, min_cost, contract_multiplier,
                     ddown_factor, leverage) -> float:
    min_entry_qty = calc_min_entry_qty(price, inverse, qty_step, min_qty, min_cost, contract_multiplier)
    qty = min(round_dn(available_margin * leverage * (price / contract_multiplier if inverse else 1 / price), qty_step),
...
@njit
def calc_available_margin(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, last_price, inverse,
                          contract_multiplier, leverage) -> float:
    used_margin = 0.0
    equity = balance
...
@njit
def iter_entries(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, liq_price, highest_bid, lowest_ask, ema,
                 last_price, volatility, inverse, do_long, do_shrt, qty_step, price_step, min_qty, min_cost,
                 contract_multiplier, ddown_factor, qty_pct, leverage, n_close_orders, grid_spacing,
                 pos_margin_grid_coeff, volatility_grid_coeff, volatility_qty_coeff, min_markup, markup_range, ema_span,
                 ema_spread, stop_loss_liq_diff, stop_loss_pos_pct, entry_liq_diff_thr):
    ...
    stop_loss_order = calc_stop_loss(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, liq_price, highest_bid,
                                     lowest_ask, last_price, available_margin, inverse, do_long, do_shrt, qty_step,
                                     min_qty, min_cost, contract_multiplier, leverage, stop_loss_liq_diff,
                                     stop_loss_pos_pct)
    ...
    if stop_loss_order[0] != 0.0:
        ...
        if 'entry' in stop_loss_order[4]:
            ...
            available_margin += calc_margin_cost(stop_loss_order[0], stop_loss_order[1], inverse, contract_multiplier,
                                                 leverage)
    while True:
        if do_long:
            if long_psize == 0.0:
                price = min(highest_bid, round_dn(ema * (1 - ema_spread), price_step))
                qty = calc_initial_entry_qty(balance, price, available_margin, volatility, inverse, qty_step, min_qty,
                                             min_cost, contract_multiplier, leverage, qty_pct, volatility_qty_coeff)
                long_entry = (qty, price, qty, price, 'initial_long_entry')
            else:
                modifier = (1 + (calc_margin_cost(long_psize, long_pprice, inverse, contract_multiplier,
                                                  leverage) / balance) * pos_margin_grid_coeff) * (
                ...
                if price <= 0.0:
                    ...
                    qty = calc_reentry_qty(long_psize, price, available_margin, inverse, qty_step, min_qty, min_cost,
        ...
        if do_shrt:
            if shrt_psize == 0.0:
                ...
                modifier = (1 + (calc_margin_cost(shrt_psize, shrt_pprice, inverse, contract_multiplier,
                                                  leverage) / balance) * pos_margin_grid_coeff) * (
                                   1 + volatility * volatility_grid_coeff)
        ...
        if long_first:
            ...
            if long_entry[1]:
                available_margin = max(0.0, available_margin - calc_margin_cost(long_entry[0], long_entry[1], inverse,
                                                                                contract_multiplier, leverage))
        else:
            yield shrt_entry
            shrt_psize, shrt_pprice = shrt_entry[2:4]
            if shrt_entry[1]:
                available_margin = max(0.0, available_margin - calc_margin_cost(shrt_entry[0], shrt_entry[1], inverse,
                                                                                contract_multiplier, leverage))


@njit
def calc_counter_order(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, liq_price, highest_bid, lowest_ask,
                       last_price, available_margin, inverse, do_long, do_shrt, qty_step, min_qty, min_cost,
                       contract_multiplier, leverage, stop_loss_liq_diff, stop_loss_pos_pct, counter_order_liq_diff):
    ...
    if calc_diff(liq_price, last_price) < counter_order_liq_diff:
        if long_psize > abs_shrt_psize:
            ...
            counter_order_qty = min(max(min_entry_qty,
                                        calc_qty_from_margin(balance * stop_loss_pos_pct, lowest_ask, inverse, qty_step,
                                                             contract_multiplier, leverage)),
                                    calc_qty_from_margin(available_margin, lowest_ask, inverse, qty_step,
                                                         contract_multiplier, leverage))
            if counter_order_qty >= min_entry_qty:
                new_shrt_psize, new_shrt_pprice = calc_new_psize_pprice(shrt_psize, shrt_pprice, -counter_order_qty,
                                                                        lowest_ask, qty_step)
            ...
            counter_order_qty = min(max(min_entry_qty,
                                    ...
                                    calc_qty_from_margin(available_margin, highest_bid, inverse, qty_step,
                                                         contract_multiplier, leverage))
            if counter_order_qty >= min_entry_qty:
                new_long_psize, new_long_pprice = calc_new_psize_pprice(long_psize, long_pprice, counter_order_qty,
                                                                        highest_bid, qty_step)
                return counter_order_qty, highest_bid, new_long_psize, new_long_pprice, 'counter_long_entry'
    return 0.0, 0.0, 0.0, 0.0, ''


@njit
def calc_stop_loss_separate(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, liq_price, highest_bid,
                            lowest_ask, last_price, available_margin, inverse, do_long, do_shrt, qty_step, min_qty,
                            min_cost, contract_multiplier, leverage, stop_loss_liq_diff, stop_loss_pos_pct):
    # returns (qty, price, psize if taken, pprice if taken, comment)
    abs_shrt_psize = abs(shrt_psize)
    if calc_diff(liq_price, last_price) < stop_loss_liq_diff:
        if long_psize > abs_shrt_psize:
            stop_loss_qty = min(long_psize, max(calc_min_entry_qty(lowest_ask, inverse, qty_step, min_qty, min_cost,
                                                                   contract_multiplier),
            ...
            stop_loss_qty = min(abs_shrt_psize,
                                max(calc_min_entry_qty(highest_bid, inverse, qty_step, min_qty, min_cost,
                                                       contract_multiplier),
...
@njit
def calc_stop_loss(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, liq_price, highest_bid, lowest_ask,
                   last_price, available_margin, inverse, do_long, do_shrt, qty_step, min_qty, min_cost,
                   contract_multiplier, leverage, stop_loss_liq_diff, stop_loss_pos_pct):
    # returns (qty, price, psize if taken, pprice if taken, comment)
    abs_shrt_psize = abs(shrt_psize)
    if calc_diff(liq_price, last_price) < stop_loss_liq_diff:
        if long_psize > abs_shrt_psize:
            ...
            margin_cost = calc_margin_cost(stop_loss_qty, lowest_ask, inverse, contract_multiplier, leverage)
            if margin_cost < available_margin and do_shrt:
                # add to shrt pos
                shrt_psize, shrt_pprice = calc_new_psize_pprice(shrt_psize, shrt_pprice, -stop_loss_qty, lowest_ask,
                                                                qty_step)
            ...
            margin_cost = calc_margin_cost(stop_loss_qty, highest_bid, inverse, contract_multiplier, leverage)
            if margin_cost < available_margin and do_long:
                # add to long pos
                long_psize, long_pprice = calc_new_psize_pprice(long_psize, long_pprice, stop_loss_qty, highest_bid,
                                                                qty_step)
...
@njit
def iter_long_closes(balance, psize, pprice, lowest_ask, inverse, do_long, do_shrt, qty_step, price_step, min_qty,
                     min_cost, contract_multiplier, ddown_factor, qty_pct, leverage, n_close_orders, grid_spacing,
                     pos_margin_grid_coeff, volatility_grid_coeff, volatility_qty_coeff, min_markup, markup_range,
                     ema_span, ema_spread, stop_loss_liq_diff, stop_loss_pos_pct, entry_liq_diff_thr):
    # yields (qty, price, psize_if_taken)
    if psize == 0.0 or pprice == 0.0:
        return
    minm = pprice * (1 + min_markup)
    prices = np.linspace(minm, pprice * (1 + min_markup + markup_range), int(n_close_orders))
    prices = [p for p in sorted(set([round_up(p_, price_step) for p_ in prices])) if p >= lowest_ask]
    if len(prices) == 0:
        ...
        for price in prices:
            if n_orders == 0:
                ...
                qty = min(psize,
                          max(calc_initial_entry_qty(balance, lowest_ask, balance, 0.0, inverse, qty_step, min_qty,
                                                     min_cost, contract_multiplier, leverage, qty_pct,
                                                     volatility_qty_coeff), round_up(psize / n_orders, qty_step)))
...
@njit
def iter_shrt_closes(balance, psize, pprice, highest_bid, inverse, do_long, do_shrt, qty_step, price_step, min_qty,
                     min_cost, contract_multiplier, ddown_factor, qty_pct, leverage, n_close_orders, grid_spacing,
                     pos_margin_grid_coeff, volatility_grid_coeff, volatility_qty_coeff, min_markup, markup_range,
                     ema_span, ema_spread, stop_loss_liq_diff, stop_loss_pos_pct, entry_liq_diff_thr):
    # yields (qty, price, psize_if_taken)
    ...
    minm = pprice * (1 - min_markup)
    prices = np.linspace(minm, pprice * (1 - (min_markup + markup_range)), int(n_close_orders))
    prices = [p for p in sorted(set([round_dn(p_, price_step) for p_ in prices]), reverse=True) if p <= highest_bid]
    if len(prices) == 0:
        yield abs_psize, min(highest_bid, round_dn(minm, price_step)), 0.0
    else:
        n_orders = int(min([n_close_orders, len(prices), int(abs_psize / min_qty)]))
        for price in prices:
            if n_orders == 0:
                break
            else:
                qty = min(abs_psize,
                          max(calc_initial_entry_qty(balance, highest_bid, balance, 0.0, inverse, qty_step, min_qty,
                                                     min_cost, contract_multiplier, leverage, qty_pct,
                                                     volatility_qty_coeff), round_up(abs_psize / n_orders, qty_step)))
...
@njit
def calc_liq_price_binance(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, inverse, contract_multiplier,
                           leverage):
...
@njit
def calc_liq_price_bybit(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, inverse, contract_multiplier,
                         leverage):
    mm = 0.005
    abs_shrt_psize = abs(shrt_psize)
    if inverse:
...
@njit
def calc_bankruptcy_price(balance, long_psize, long_pprice, shrt_psize, shrt_pprice, inverse, contract_multiplier):
    long_pprice = nan_to_0(long_pprice)
    shrt_pprice = nan_to_0(shrt_pprice)
